...
export default function example() {
  ...
  // BufferGeometry는 이제껏 했던 모양이 이미 만들어진 Geometry와 달리 내가 임의로 포인트를 설정해서 그 포인트를 이어주면서 Geometry를 직접 그린 것임
  const points = [];
  ...
  function draw() {
    ...
    // = const direction = new THREE.Vector3(0, 0, -1); // 이 경우 normalize로 정교화 불필요
    direction.normalize();
    raycaster.set(origin, direction);

    // raycaster에 맞은 Mesh가 있는지 체크하여 있다면 컬러를 red로 바꿀 것
