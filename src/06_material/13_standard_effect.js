...
export default function example() {
  ...
  const normalTex = textureLoader.load("/textures/metal/Metal_006_normal.jpg");
  const roughnessTex = textureLoader.load(
    "/textures/metal/Metal_006_roughness.jpg"
  );
  const heightTex = textureLoader.load("/textures/metal/Metal_006_height.png");
  ...
  const material = new THREE.MeshStandardMaterial({
    map: baseColorTex,
    normalMap: normalTex, // 디테일한 차이
    roughnessMap: roughnessTex, // 빛이 들어올 때의 차이
    metalness: 0.3,
    aoMap: ambientOcclusionTex, // shadow를 진하게
  });
