...
export default function example() {
  ...
  for (let i = 0; i < spherePositionArray.length; i += 3) {
    imagePanel = new ImagePanel({
      ...
      imageSrc: `/textures/particle/0${Math.ceil(Math.random() * 5)}.jpg`,
  ...
  const setShape = (e) => {
    ...
    switch (type) {
      case "random":
        // randomPositionArray 사용
        array = randomPositionArray;

        break;
      case "sphere":
        // spherePositionArray 사용
  ...
  sphereBtn.textContent = "모으기";
  sphereBtn.style.cssText = `position:absolute; top:50px; left:20px`;
  btnWrapper.append(randomBtn, sphereBtn);
